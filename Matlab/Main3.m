...
%% 初始化
% p=[Fkine([0,-5,-30,10,-23,34]);
%     Fkine([12,-23,34,-45,56,-67]);
%     Fkine([12,-23,34,-45,56,-67])];
p_target=[200,-400,350,0,0,0;%起点笛卡尔坐标xyzabc
    850,300,600,0,0,0;%中点笛卡尔坐标xyzabc
    1000,800,300,0,0,0];%起点笛卡尔坐标xyzabc
...
for i=1:size(p_target,1)-1
    
    p_start=p_target(i,1:3);
    p_stop=p_target(i+1,1:3);
    p_list1=[p_start,0,0];
    p_list2=[p_stop,0,0];
    ...
    for j=1:loop
        treenum=mod(j,2);%每个loop只操作一棵树，两个树轮换操作
        ...
        if finish
            ...
            p_collect(i).list=Collect(p_list1,p_list2,i_touch,treenum);%初始路径列表
            p_simple(i).list=Simple(p_collect(i).list, obstacle);%简化路径列表
            p_spline(i).list=Bspline(p_simple(i).list);%三次B样条平滑路径列表
            %     p_optimize = Optimize(p_collect);
            plot3(p_collect(i).list(:,1), p_collect(i).list(:,2), p_collect(i).list(:,3),'y-', 'Linewidth', 2);
            plot3(p_simple(i).list(:,1), p_simple(i).list(:,2), p_simple(i).list(:,3),'g-', 'Linewidth', 2);
            plot3(p_spline(i).list(:,1), p_spline(i).list(:,2), p_spline(i).list(:,3),'c-', 'Linewidth', 2);
            %     plot3(p_optimize(:,1), p_optimize(:,2), p_optimize(:,3),'g-', 'Linewidth', 2);
            %     PlotResult(p_collect);
            fprintf('%d segment all point-number is %d\nfinish\n', i, size(p_simple(i).list,1));
            break;
        end
